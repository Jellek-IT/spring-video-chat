...
int janus_videoroom_init(janus_callbacks *callback, const char *config_path) {
    ...
    /* Modified by Maciej Bronikowski
       log that changes were actually included
       Start */
	   JANUS_LOG(LOG_INFO, "Running modified version of plugin by Maciej Bronikowski\n");
	/* Modified by Maciej Bronikowski
       END */
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				/* Modified by Maciej Bronikowski
				   If allowed list is used, user id is created based on allowed string, if not, it is generated.
				   User cannot pass own id
                   Start */
				char user_id_num[30], *user_id_str = NULL;
				gboolean user_id_allocated = FALSE;
				...
				if(videoroom->check_allowed) {
					...
					/* set user id to value from token if string ids */
					if(string_ids) {
                        gchar **list = g_strsplit(token_text, ";", 2);
                        if(list[0] != NULL && list[1] != NULL) {
                            user_id_str = g_strdup(list[0]);
                            user_id_allocated = TRUE;
                        }
                        g_clear_pointer(&list, g_strfreev);
                        if(g_hash_table_lookup(videoroom->participants, (gpointer)user_id_str) != NULL) {
                            /* User ID already taken */
                            janus_mutex_unlock(&videoroom->mutex);
                            janus_mutex_unlock(&sessions_mutex);
                            janus_refcount_decrease(&videoroom->ref);
