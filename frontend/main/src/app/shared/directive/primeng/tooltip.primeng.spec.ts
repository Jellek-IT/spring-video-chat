/**
 * for primeng version 19.0.5 there is bug where with appendTo option wrong element is referenced during removeChild
 * this is newer version from commit https://github.com/primefaces/primeng/commit/b93a4e18a3b3c07880de8cd4af20d82384fd39ea
 * that has this bug fixed
 * todo: delete when new version of primeng will be released
 */
...
export class Tooltip extends BaseComponent implements AfterViewInit, OnDestroy {
  ...
  override ngOnChanges(simpleChange: SimpleChanges) {
  ...
  onMouseLeave(e: MouseEvent) {
    if (!this.isAutoHide()) {
      const valid =
        hasClass(e.relatedTarget as any, 'p-tooltip') ||
        hasClass(e.relatedTarget as any, 'p-tooltip-text') ||
        hasClass(e.relatedTarget as any, 'p-tooltip-arrow');
  ...
  updateText() {
    const content = this.getOption('tooltipLabel') as any;
    if (content instanceof TemplateRef) {
      const embeddedViewRef = this.viewContainer.createEmbeddedView(content);
      embeddedViewRef.detectChanges();
      embeddedViewRef.rootNodes.forEach((node) =>
        this.tooltipText.appendChild(node)
      );
  ...
  private get activeElement(): HTMLElement {
    return this.el.nativeElement.nodeName.includes('P-')
      ? findSingle(this.el.nativeElement, '.p-component')
  ...
  alignLeft() {
    ...
    let offsetTop =
      (getOuterHeight(this.el.nativeElement) - getOuterHeight(this.container)) /
      2;
    this.alignTooltip(-offsetLeft, offsetTop);
  }
  ...
  alignTop() {
    this.preAlign('top');
    let offsetLeft =
      (getOuterWidth(this.el.nativeElement) - getOuterWidth(this.container)) /
      2;
